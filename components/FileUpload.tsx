...
const authenticator = async () => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to authenticate the request: ${response.status}: ${errorText}`
      );
    }
    ...
    throw new Error(
      `An error occurred while authenticating the request: ${error.message}`
    );
  }
};
...
const FileUpload = ({
  ...
  return (
    ...
    >
      ...
      <button
        className={cn("upload-btn bg-gray-800", styles.button)}
        onClick={(e) => {
          e.preventDefault();
